...
const COLUMN_ALIASES: Record<string, string> = {
  variable_name: "variable_name", "değişken adı": "variable_name", degisken: "variable_name", variablename: "variable_name",
  unit_name: "unit_name", "birim adı": "unit_name", birim: "unit_name", unit: "unit_name",
  sub_unit_name: "sub_unit_name", "alt birim": "sub_unit_name", alt_birim: "sub_unit_name",
  year: "year", yil: "year", "yıl": "year",
  month: "month", ay: "month",
  value: "value", deger: "value", "değer": "value",
  note: "note", not: "note", notlar: "note", aciklama: "note", "açıklama": "note", source: "note",
};
...
function validateRow(row: Record<string, unknown>, idx: number): string | null {
  if (!String(row.variable_name ?? "").trim()) return `Satır ${idx + 1}: Değişken adı (variable_name) zorunludur`;
  ...
  if (!month || month < 1 || month > 12) return `Satır ${idx + 1}: Geçersiz ay (1-12): ${row.month}`;
  const val = parseFloat(String(row.value ?? ""));
  if (isNaN(val)) return `Satır ${idx + 1}: Geçersiz değer (${row.value})`;
  return null;
}

function generateTemplate(): void {
  const rows = [
    { variable_name: "Üretim Miktarı", unit_name: "Merkez Birim", sub_unit_name: "Üretim Hattı 1", year: 2024, month: 1, value: 1250, note: "Ocak üretimi" },
    { variable_name: "Çalışan Sayısı", unit_name: "Merkez Birim", sub_unit_name: "", year: 2024, month: 1, value: 85, note: "" },
  ];
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Değişken Değerleri");
  XLSX.writeFile(wb, "degisken_deger_sablonu.xlsx");
}
...
export default function VariableValueImport({ open, onOpenChange }: Props) {
  const { token } = useAuth();
  ...
  async function handleImport() {
    ...
    setImporting(true);
    try {
      const res = await fetch("/api/variable-values/batch", {
        method: "POST",
      ...
      if (data.imported > 0) {
        toast({ title: `${data.imported} değer başarıyla içe aktarıldı` });
      }
    } catch {
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Toplu Değişken Değeri İçe Aktar
          </DialogTitle>
        </DialogHeader>
            ...
            <>
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  Zorunlu:{" "}
                  <span className="font-mono text-xs bg-muted px-1 rounded">variable_name</span>{" "}
                  <span className="font-mono text-xs bg-muted px-1 rounded">year</span>{" "}
                  <span className="font-mono text-xs bg-muted px-1 rounded">month</span>{" "}
                  <span className="font-mono text-xs bg-muted px-1 rounded">value</span>
                </p>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="text-xs">
                          <TableHead className="py-2">Değişken</TableHead>
                          ...
                          <TableHead className="py-2 text-right">Değer</TableHead>
                      ...
                      <TableBody>
                        {previewRows.map((r, i) => (
                          <TableRow key={i} className="text-xs">
                            <TableCell className="py-1.5 font-medium max-w-[120px] truncate">{String(r.variable_name ?? "")}</TableCell>
                            ...
                            <TableCell className="py-1.5 text-right font-mono">{String(r.value ?? "")}</TableCell>
                            <TableCell className="py-1.5 max-w-[80px] truncate text-muted-foreground">{String(r.note ?? "")}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
